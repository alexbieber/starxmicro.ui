...
import { AboutDocumentation } from "./components/AboutDocumentation";
...
type AppPage = "shader" | "browse" | "installation" | "about" | "not-found";
...
type ResolvedRoute = {
  browseCategory?: ReadyShader["category"];
  browseTag?: string;
  page: AppPage | "oauth-consent" | "pricing" | "mcp";
  canonicalPath: string;
  shader?: ReadyShader;
  variantId?: string;
  legacy?: boolean;
...
function routeStateFromUrl(): RouteState {
  ...
  return {
    ...
    routedVariantId,
    page: route.page === "about" || route.page === "mcp"
      ? "about"
      : route.page === "oauth-consent" || route.page === "pricing"
        ? "not-found"
        : route.page as AppPage,
...
function ShaderCapturePage() {
  ...
  useEffect(() => {
    applyAppearance("dark", "star", "star");
    applyRouteSeo({
      page: "capture",
      shader,
      variant,
...
function StarXApp() {
  ...
  const selectAbout = () => {
    setRouteState((current) => ({ ...current, page: "about", canonicalPath: STATIC_ROUTE_PATHS.about }));
    setSidebarOpen(false);
    window.history.pushState({}, "", navigationUrl(STATIC_ROUTE_PATHS.about));
  };
  ...
  return (
    <>
      <header className="topbar">
        ...
        <button className="topbar-brand-button" aria-label="Browse StarX UI" onClick={selectBrowse}>
          <BrandMark compact />
        </button>
        <div className="topbar-actions">
          <UpgradeLink compact onInstall={selectInstallation} />
        ...
        <div className="pane">
          <div className="pane-scroll scroll-area">
            {page === "browse" ? (
              ...
              <AboutDocumentation onBrowse={selectBrowse} onInstallation={selectInstallation} />
            ) : page === "not-found" ? (
              <main className="browse-page" aria-labelledby="not-found-title">
                <header className="browse-header">
                  <h1 id="not-found-title">Page not found</h1>
                  <p className="lede">This StarX UI URL does not match a component, variant, or documentation page.</p>
...
export default function App() {
  return captureFromUrl() ? <ShaderCapturePage /> : <StarXApp />;
}
