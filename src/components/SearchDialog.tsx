...
export function SearchDialog({ open, initialQuery = "", onClose, onSelect }: SearchDialogProps) {
  ...
  return (
    <>
      ...
      <div ref={dialogRef} className="dialog card open" role="dialog" aria-modal="true" aria-label="Search components">
        <div className="field">
        ...
        <div className="results" onScroll={() => {
          ...
          {results.length ? (
            results.map((result) => {
              ...
              const preview = variant?.preview ?? shader.preview ?? `/previews/${shader.id}.webm`;
              return (
                <button
                  key={resultId}
            ...
            <div className="empty">No matching component found.</div>
          )}
        </div>
        {tooltip ? (
          <div
