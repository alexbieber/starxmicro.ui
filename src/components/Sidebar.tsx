...
import type { ReadyShader, ReadyShaderCategory } from "../data/shaders";
import { READY_SHADER_CATEGORIES } from "../data/shaders";
...
export function Sidebar({ active, browseActive, installationActive, aboutActive, open, theme, palette, onSelect, onBrowse, onInstallation, onAbout, onSearch, onTheme }: SidebarProps) {
    ...
    <aside className={`sidebar${open ? " open" : ""}`} id="sidebar">
      <div className="sb-head">
        <div className="sb-brand-row">
          <button className="brand-button" aria-label="Browse StarX UI" onClick={onBrowse}>
            <BrandMark />
          </button>
        </div>
        <button className="search inset-shadow" onClick={onSearch}>
          <SearchIcon style={{ opacity: 0.7 }} />
          <span className="ph">Search components</span>
          <kbd>⌘ K</kbd>
        </button>
      </div>
      <div className="sb-scroll scroll-area" onScroll={() => setPreview(null)}>
        <nav className="sb-nav" aria-label="StarX UI documentation">
          <div className="nav-section">
            ...
            <div className="nav-list nav-list-root">
              ...
              <button
                className={`nav-link${aboutActive ? " active" : ""}`}
          ...
          <SidebarCatalogSection
            ...
            active={active}
            installationActive={installationActive || aboutActive || browseActive}
            onSelect={onSelect}
            onPreview={showPreview}
            onPreviewEnd={() => setPreview(null)}
          />
        </nav>
        <div className="sidebar-promos-mobile">
          <RightRailPromos onInstall={onInstallation} />
        </div>
      </div>
      <div className="sb-foot">
        <UpgradeLink onInstall={onInstallation} />
