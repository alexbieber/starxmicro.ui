...
import { BRAND_NAME, PACKAGE_NAME, SITE_DESCRIPTION, SITE_TITLE } from "./brand";
...
export function buildRouteSeo(route, origin, catalog = [], mediaOptions) {
  const defaultImage = absoluteMediaUrl(origin, "/starx-mark.svg", mediaOptions);
  ...
  if (route.page === "installation") {
    title = brandedTitle(`Install ${BRAND_NAME} for React`);
    description = `Install ${PACKAGE_NAME}, add the shared styles, check peer requirements, and verify your first interactive Three.js component.`;
    structuredData = {
      "@context": "https://schema.org",
      "@type": "TechArticle",
      headline: `Install ${BRAND_NAME} for React`,
      description,
      url: absoluteUrl(origin, canonicalPath),
      about: { "@type": "SoftwareApplication", name: `${BRAND_NAME} for React`, applicationCategory: "DeveloperApplication" },
    };
  } else if (route.page === "about" || route.page === "mcp") {
    title = brandedTitle(`About ${BRAND_NAME}`);
    description = `${BRAND_NAME} is an open catalog of live Three.js components, forked from ThreeUI Community. Browse previews, copy source, and ship.`;
    structuredData = {
      "@context": "https://schema.org",
      "@type": "TechArticle",
      headline: `About ${BRAND_NAME}`,
      description,
      url: absoluteUrl(origin, canonicalPath),
      about: { "@type": "SoftwareApplication", name: BRAND_NAME, applicationCategory: "DeveloperApplication" },
    };
    ...
    description = "The requested StarX UI page could not be found. Browse the complete component collection instead.";
  } else {
    ...
    structuredData = {
      ...
      hasPart: [
        ...
        { "@type": "TechArticle", name: "About", url: absoluteUrl(origin, STATIC_ROUTE_PATHS.about) },
        ...(browseFaqs.length > 0
...
export function applyRouteSeo(route, catalog = []) {
  ...
  let structuredData = document.head.querySelector("#starx-structured-data");
  if (seo.structuredData) {
    if (!structuredData) {
      structuredData = document.createElement("script");
      structuredData.id = "starx-structured-data";
