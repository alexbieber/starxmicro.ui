...
test("installation surfaces reference the published npm package", async () => {
  ...
  assert.match(sources[0], /PACKAGE_NAME/);
  assert.match(sources[1], /npm install \$\{PACKAGE_NAME\}/);
  assert.match(sources[1], /\$\{PACKAGE_NAME\}\/style\.css/);
  assert.match(sources[2], /from "\$\{PACKAGE_NAME\}"/);
  assert.match(sources[4], /Install \$\{PACKAGE_NAME\}/);
...
test("the public shell keeps the main UI but has no auth or private catalog runtime", async () => {
  ...
  assert.match(upgrade, /STATIC_ROUTE_PATHS\.installation/);
